...
Le packaging est porté par le `NonFoodStockBucket`. L'approvisionnement crée un bucket portant le packaging, puis enregistre un `SupplyMovement` avec une ligne positive sur ce bucket.
...
SupplyMovement
└── StockMovementLine (QuantityDelta > 0) → NonFoodStockBucket
```
...
Elle est portée par le `FoodStockBucket`. L'approvisionnement crée un bucket portant la DLC, puis enregistre un `SupplyMovement` avec une ligne positive sur ce bucket.
...
SupplyMovement
└── StockMovementLine (QuantityDelta > 0) → FoodStockBucket
```
...
Le mouvement global ne porte plus ni DLC ni packaging : ces informations appartiennent aux `StockBucket`. Les impacts chiffrés sont enregistrés dans `StockMovementLines`.

Tables principales :

```text
Articles                  (TPH : Food / NonFood)
FoodArticleSaleModes
ArticleMovements          (TPH : Supply / Sale / Inventory, sans DLC ni packaging)
StockBuckets              (TPH : Food / NonFood, porte DLC ou packaging)
StockMovementLines        (delta, quantités avant/après, FK Bucket + Movement)
```
